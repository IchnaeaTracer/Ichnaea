...
#define MAGENTA_SHELL "\033[1;35m"
		...
		PostPrinter_t postprinter, const uint8_t *code)
{
	...
	*mnem = '\0';
	...
	if (handle->mnem_list) {
		printf(MAGENTA_SHELL "Addr of HANDLE:%p\n" END_COLOR , handle);
		printf(MAGENTA_SHELL "Addr of mnem_list:%p\n" END_COLOR , handle->mnem_list);
			...
			if (opt->id) {
				if (opt->mnemonic) {
					...
					if (!tmp) {
						...
						printf(MAGENTA_SHELL "Addr of tmp1:%p\n" END_COLOR , tmp);
						handle->mnem_list = tmp; // MOD point #1
					}
					return CS_ERR_OK;
				} else {
					...
					while(tmp) {
						if (tmp->insn.id == opt->id) {
							...
							if (tmp == prev) {
								// head of the list
								printf(MAGENTA_SHELL "Addr of tmp2:%p\n" END_COLOR , tmp->next);
								handle->mnem_list = tmp->next; // MOD point #2
							} else {
								prev->next = tmp->next;
							}
							cs_mem_free(tmp);
